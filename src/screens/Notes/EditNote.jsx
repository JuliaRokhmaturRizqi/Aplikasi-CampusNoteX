// src/screens/Notes/EditNote.jsx
...
export default function EditNote({ route }) {
  ...
  const incoming = route?.params?.note ?? {};

  const [category, setCategory] = useState(incoming.category || "Revisi");
  const [title, setTitle] = useState(incoming.title || "");
  const [body, setBody] = useState(incoming.body || "");
  const [attachment, setAttachment] = useState(incoming.attachment || null);
  ...
  useEffect(() => {
    const show = Keyboard.addListener("keyboardDidShow", (e) => {
      ...
      // beri offset kecil supaya toolbar lebih atas dari keyboard
  ...
  // simple storage helper (kept same behavior as before)
  const saveNoteToStorage = async (noteObj) => {
    try {
      const raw = await AsyncStorage.getItem(STORAGE_KEY);
      const arr = raw ? JSON.parse(raw) : [];
      if (noteObj.id) {
        const idx = arr.findIndex((n) => n.id === noteObj.id);
        if (idx >= 0) arr[idx] = noteObj;
        else arr.unshift(noteObj);
      } else {
  ...
  const handleSave = async () => {
    ...
    const ok = await saveNoteToStorage(updated);
    if (ok) navigation.navigate("NoteDetail", { note: updated });
    else Alert.alert("Gagal", "Gagal menyimpan catatan. Coba lagi.");
  };
  ...
  // wrap selected text with pre/suf OR insert empty markers and place caret between
  const wrapSelection = (pre, suf) => {
    ...
    if (start < end) {
      ...
      setBody(newBody);
      // set selection after a tiny delay to ensure TextInput updated
      setTimeout(() => {
  ...
  // insert image token and set attachment
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.titleCard}>
          <View style={styles.titleCardHeader}>
            <Text style={styles.titleCardHeaderText}>Ubah Judul Catatan</Text>
          </View>
